...
function getDbPath(host) {
  // Extract database name from host (e.g., "myapp.localhost" -> "myapp")
  const dbName = host.split('.')[0];
  return path.join(DB_DIR, `${dbName}.json`);
}

// Helper function to check if database exists
async function databaseExists(host) {
  try {
    const dbPath = getDbPath(host);
...
async function startServer() {
  ...
  app.listen(PORT, () => {
    console.log(`🚀 Mini Supabase Server running on http://localhost:${PORT}`);
