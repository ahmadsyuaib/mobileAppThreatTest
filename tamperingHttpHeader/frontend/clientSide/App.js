    ...
    try {
      ...
      const response = await fetch(`${serverUrl}/api/auth`, {
        ...
        headers: {
          ...
          host: host.trim(), // Custom host header as requested
        },
        body: JSON.stringify({
      ...
      if (response.ok) {
        setResponse(`Success! User added:\n${JSON.stringify(data, null, 2)}`);
  ...
  const createDatabase = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert('Error', data.message || 'Failed to create database');
      }
    } catch (error) {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          >
            <Text style={styles.createDbButtonText}>Create Database</Text>
          </TouchableOpacity>
        </View>
